 ...
 * lo aplica en el WHERE. No es un detalle de estilo: el módulo origen tenía
...
import type { MessageLog, NewMessageLog } from '@/db/schema/message-logs';
import type { WabaConfig } from '@/db/schema/waba-configs';
...
/** Descifra el token y devuelve la conexión completa (solo uso interno). */
function hydrate(row: WabaConfig): WabaConnection {
  return {
    ...
    accessToken: decryptSecret(row.encryptedToken),
 ...
 *
 * Nunca la llames desde una Server Action ni desde código con sesión.
 */
...
export async function upsertConnection(
  ...
  if (!row) throw new Error('[WABA] upsertConnection: returning() vacío.');
...
export async function touchConnectionSync(
  organizationId: OrganizationId,
  connectionId: string,
  patch: { displayPhone?: string | null; verifiedName?: string | null }
): Promise<void> {
  await db
    .update(wabaConfigs)
    .set({
      // displayPhone es NOT NULL en schema → coalesce a '' si viene null
      ...(patch.displayPhone !== undefined
        ? { displayPhone: patch.displayPhone ?? '' }
        : {}),
...
export interface UpsertMessageEventInput {
  ...
  /** El wamid de Meta (equivalente a message_id del export module). */
  wamid: string;
...
/**
 * Upsert idempotente sobre `wamid`.
 *
 ...
 *
 * ⚠️ Requiere el índice único parcial `message_logs_wamid_uq`.
...
export async function upsertMessageEvent(
  ...
  if (!input.wamid) {
    console.warn('[WABA] Evento sin wamid descartado.');
    return;
  }
  ...
  await db
    ...
    .values(values)
    .onConflictDoUpdate({
      target: messageLogs.wamid,
      targetWhere: sql`${messageLogs.wamid} IS NOT NULL`,
      set: {
...
export async function getMessageEventStatus(
  ...
  if (!organizationId || !wamid) return null;
...
export async function getDeliveryStats(
  ...
  const rows = await db
    .select({
      status: messageLogs.deliveryStatus,
    ...
    .groupBy(messageLogs.deliveryStatus);
 ...
 * Busca por los últimos 8 dígitos (móvil boliviano) para tolerar
 * que la base guarde `71234567` y Meta devuelva `59171234567`.
 */
 ...
 * Requiere opt-in de WhatsApp (Meta lo exige).
 */
 ...
 * El webhook SIEMPRE responde 200 a Meta aunque el insert fallara (GOTCHAS G-11).
 */
