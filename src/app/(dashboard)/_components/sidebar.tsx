...
export function Sidebar({ user }: { user?: UserSession | null }) {
  ...
  // Super Admin Platform Control Group
  const superAdminGroup: NavGroup = {
    groupName: 'Plataforma Super Admin',
    items: [
      {
        label: 'Tenants / ISPs',
  ...
  // ISP Operations Group (Available to ISP Users AND Super Admin for testing/management)
  const ispOperationsGroup: NavGroup = {
    groupName: 'Gestión de ISP & Cartera',
    items: [
      {
  ...
  // WhatsApp & Bot IA Group
  const whatsappIaGroup: NavGroup = {
    groupName: 'WhatsApp & Bot IA',
    items: [
      {
      ...
      },
      {
        ...
        href: '/settings/automations/logs',
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const configGroup: NavGroup = {
    groupName: 'Configuración & Cuenta',
    items: [
      {
  ...
  // Super Admin gets ALL groups; normal tenant admin gets ISP + WhatsApp + Config
  const navGroups: NavGroup[] = isSuperAdmin
    ? [superAdminGroup, ispOperationsGroup, whatsappIaGroup, configGroup]
    : [ispOperationsGroup, whatsappIaGroup, configGroup];
