 ...
 *
 * Diferencias frente al route handler original (`api/waba/connect/route.ts`):
 *   ✅ Exige sesión: el tenant sale de la sesión, nunca del body
 *   ✅ Cifra el token antes de guardarlo (GOTCHAS G-06)
 *   ✅ Suscribe la app al WABA automáticamente (GOTCHAS G-17)
...
/** Rate limit en memoria. En multi-instancia usa Redis. */
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      displayPhone = profile.display_phone_number ?? null;
    ...
    // 6. Suscribir la app al WABA (GOTCHAS G-17)
