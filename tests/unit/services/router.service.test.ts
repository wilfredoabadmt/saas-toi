...
vi.mock('@/lib/mikrotik/client', () => ({
  MikroTikClient: {
    ...
    setSubscriberServiceStatus: vi.fn().mockResolvedValue({
      status: 200,
      command: 'PATCH https://192.168.88.1:443/rest/ppp/secret',
...
describe('RouterService Unit Tests', () => {
  ...
  it('create should encrypt password before storing', async () => {
    const created = await RouterService.create(orgId, {
      name: 'MikroTik CCR2004',
      host: '192.168.88.1',
      username: 'admin',
    ...
    expect(created.name).toBe('MikroTik CCR2004');
  });

  it('testConnection should decrypt password and run MikroTikClient test', async () => {
