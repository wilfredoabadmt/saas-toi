...
describe('allowlist de categorías de ticket', () => {
  it('resuelve variantes con acento, mayúscula y espacio', () => {
    expect(resolveTicketCategory('Equipo Dañado')).toBe('equipo_danado');
...
describe('render del estado de cuenta', () => {
  ...
  it('normaliza los estados de servicio del ISP', () => {
    expect(normalizeServiceStatus('suspended', 'overdue')).toBe('cortado');
    expect(normalizeServiceStatus('active', 'current')).toBe('activo');
    expect(normalizeServiceStatus('lo que sea', null)).toBe('desconocido');
  });
...
const perfil = {
  ...
  greeting: null,
  paymentInstructions: 'Transferencia Cta 100000',
...
describe('system prompt del agente', () => {
  ...
  it('inyecta identidad, fecha, estado de cuenta y formas de pago', () => {
    const p = buildAgentSystemPrompt({
      profile: perfil,
      kb: [{ kind: 'qa', question: '¿Cobertura?', answer: 'El Alto y La Paz', content: null }],
      account: cuenta,
      today,
    });
    expect(p).toContain('Asistente TOI');
    expect(p).toContain('Hoy es 2026-07-25');
    expect(p).toContain('998.00');
    expect(p).toContain('Transferencia Cta 100000');
    expect(p).toContain('El Alto y La Paz');
